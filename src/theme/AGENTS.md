...
`ThemeProvider.tsx` owns the persisted palette mode, appearance preset, and motion intensity state, then delegates all theme resolution to `createAppTheme.ts`, which exposes the resolved values on `theme.appearanceTreatment.{surface,motion,motionScale}` for downstream builders and components.
...
- Components and style builders consume resolved values from `theme.appearanceTreatment`; they do not reconstruct preset resolution from raw appearance records.
