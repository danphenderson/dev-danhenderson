...
- `components.tsx` — animated React primitives built on the above: `MotionSection`, `StaggerChildren`, `MotionCard`, `MotionImage`, `MotionItem`. These wrap `motion/react` with IntersectionObserver-triggered entry and sensible defaults.
...
- New animated primitives (wrappers over `motion/react` elements) go in `components.tsx`.
