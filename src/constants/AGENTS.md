...
Build-time stable configuration: route definitions, runtime-environment helpers, command palette action registry, and the not-found recovery scoring logic.
...
- `runtimeEnvironment.ts` — runtime-environment resolution via `resolveAppRuntimeEnvironment()` and `appRuntimeEnvironment`, used by build/test workflows and environment-sensitive client behavior.
...
- Route exposure, primary navigation, command-palette entries, and not-found recovery should stay derived from `siteRoutes.ts`, `routeActions.ts`, and `commandPaletteActions.ts`. Do not duplicate route inclusion logic in pages or components.
...
- If future work introduces environment-sensitive route exposure, centralize it so route JSX, primary navigation, command-palette entries, and not-found recovery stay consistent.
