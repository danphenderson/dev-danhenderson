...
- Preserve the documented motion, styling, and route/runtime invariants defined in `docs/engineering/agent-guide.md`.
...
- `npm run build:e2e` is the correct build variant for Playwright Chromium coverage when the touched behavior depends on the test runtime because it sets `REACT_APP_RUNTIME_ENV=test`.
